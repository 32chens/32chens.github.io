...
**更改缓冲区**（针对于**非唯一二级索引页**），在执行DML语句时，如果这些数据Page**不在Buffer Pool中**，不会直接操作磁盘，而会将数据变更存在更改缓冲区 Change Buffer 中，在未来数据**被读取时，再将数据合并恢复到Buffer Pool中**，再定期刷磁盘，而不是每次刷磁盘，能够降低磁盘IO，提升MySQL的性能。（合并写操作）
